...
// -indexed array a
//  of size n
//  is called good if for all valid indices i
//  (0≤i≤n−1
// ), ai+i
//  is a perfect square†
// // .
...
void solve()
{
    ...
    int maxa=2*(n-1);
    vector<int> square;
    for(int i=0;i*i<=maxa;i++)  square.pb(i*i);
    int ptr=square.size()-1;
    set<int> S;
    int ans[n];
    rev(i,n-1,0)
    {
        int other= square[ptr]-i;
       // cout<<other<<" "<<square[ptr]<<" "<<i<<endl;
        while(other>=n || S.find(other)!=S.end())
                ptr--,other=square[ptr]-i;
        ans[other]=i;
        S.insert(other);
    }
